...
u64int do_fork(void)
{
	...
	kprintf("Created blank process\n");
	copy_paging_structures(CURRENT_TASK, child_task_ptr);
	kprintf("Created paging structures\n");
...
void copy_paging_structures(task_struct* parent_task, task_struct* child_task) 
{
	...
	while(parent_vma_ptr != NULL){
		for(i4 = parent_vma_ptr->vm_start; i4 < parent_vma_ptr->vm_end; i4 += PML4_RANGE){
			...
			set_present(&pml_entries_ptr[i4]);
			for(i3 = i4; i3 < i4+PML4_RANGE; i3 += PDP_RANGE){
				...
				for(i2 = i3; i2 < i3+PDP_RANGE; i2 += PD_RANGE){
					...
					set_base_addr(&pdp_entries_ptr[PT_OFFSET(i2)], pt_entries_phys_addr);
					set_present(&pdp_entries_ptr[PT_OFFSET(i2)]);
					for(i1 = i2; i1 < i2+PD_RANGE; i1+= PT_RANGE){
						/* For each pt entry, copy it!*/
						pt_entries_ptr[PT_OFFSET(i1)] = *(pt_e*)(PT_ENTRY(i1)+PT_OFFSET((u64int)i1));
					}
				}
			}
		}
